...
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| 1   | The hardcoded DSN sent every error to project `4509940389707776` on org `o267366`. Source maps, weekly reports and alerts all belong to `andres-suarez-dev/portfolio` (`4510980472438784`). | The Vercel build log uploads maps to `Organization: andres-suarez-dev, Projects: portfolio`. The weekly report lists that org's three projects, and the DSN's isn't one of them. The project IDs decode to creation dates: 31 Aug 2025 for the DSN's, 3 Mar 2026 for `portfolio`, the day Sentry was re-integrated. | DSN moved to `.env.production`, now pointing at `portfolio`.   |
...
The DSN now points at `portfolio`. Its org, `o4510980468572160`, was created
on 3 Mar 2026, a minute before the project, so the old DSN's `o267366` was a
separate, older org. Anything reported before the swap went there.

Two things only the Sentry dashboard can do:

1. **Check there's an alert rule.** Alerts → Alert Rules, filtered to
